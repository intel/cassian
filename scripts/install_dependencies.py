...
def install_opencl(install_directory, tmp_directory, architecture, clear=False):
    ...
    subprocess.run(
        "git clone https://github.com/KhronosGroup/OpenCL-Headers.git --depth 1 --branch v2020.06.16",
        cwd=tmp_directory,
        shell=True,
        check=True,
    )
...
def install_level_zero(install_directory, tmp_directory, architecture, clear=False):
    ...
    subprocess.run(
        "git clone https://github.com/oneapi-src/level-zero.git --depth 1 --branch v1.0",
        cwd=tmp_directory,
        shell=True,
        check=True,
    )
