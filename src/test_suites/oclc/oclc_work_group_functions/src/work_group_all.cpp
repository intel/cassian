...
void test_work_group_all(const TestConfig &config) {
  ca::Runtime *runtime = config.runtime();
  const std::string program_type = config.program_type();

  const ca::Kernel kernel =
      create_kernel("kernels/oclc_work_group_functions/work_group_all.cl",
...
TEST_CASE("work_group_all", "") { test_work_group_all(get_test_config()); }
