...
std::string get_kernel_path(const AddressSpace address_space) {
  return std::string("kernels/oclc_vector_load_store_functions/vstore_") +
...
std::string get_build_options(const AddressSpace address_space) {
  ...
  build_options += " -D FUNCTION=vstore" +
...
run_kernel(const ca::Kernel &kernel, const size_t global_work_size,
           const size_t local_work_size, const std::vector<VECTOR_TYPE> &input,
  ...
  ca::Buffer input_buffer =
      runtime->create_buffer(sizeof(VECTOR_TYPE) * input.size());
  ...
  ca::Buffer output_buffer = runtime->create_buffer(
      sizeof(SCALAR_TYPE) * VECTOR_TYPE::vector_size * global_work_size);
  ...
  std::vector<SCALAR_TYPE> output =
      runtime->read_buffer_to_vector<SCALAR_TYPE>(output_buffer);
...
template <typename SCALAR_TYPE, typename VECTOR_TYPE>
std::vector<SCALAR_TYPE> get_reference(const std::vector<VECTOR_TYPE> &input,
                                       const size_t global_work_size) {
  const size_t element_count = VECTOR_TYPE::vector_size;
  std::vector<SCALAR_TYPE> output(element_count * global_work_size);
  for (size_t i = 0; i < global_work_size; ++i) {
    for (size_t j = 0; j < element_count; ++j) {
      const size_t offset = i * element_count + j;
      output[offset] = input[i][j];
    }
  }
  return output;
}
...
template <typename SCALAR_TYPE, typename VECTOR_TYPE>
void run_test(const std::vector<VECTOR_TYPE> &input, const ca::Kernel &kernel,
              const size_t global_work_size, const size_t local_work_size,
              const AddressSpace address_space, ca::Runtime *runtime) {
  const std::vector<SCALAR_TYPE> output = run_kernel<SCALAR_TYPE>(
      kernel, global_work_size, local_work_size, input, address_space, runtime);
  const std::vector<SCALAR_TYPE> reference =
      get_reference<SCALAR_TYPE>(input, global_work_size);
  REQUIRE_THAT(output, Catch::Equals(reference));
}

template <typename T>
void test_vstore(const TestConfig &config, const AddressSpace address_space) {
  ...
  std::vector<vector_host_type> input(global_work_size);
  std::iota(input.begin(), input.end(), vector_host_type(0));

  run_test<scalar_host_type>(input, kernel, global_work_size, local_work_size,
                             address_space, runtime);

  runtime->release_kernel(kernel);
}

TEMPLATE_LIST_TEST_CASE_CUSTOM_NAME("vstore_global", "", ca::VectorTypes,
                                    test_name<TestType>) {
  ...
  test_vstore<TestType>(get_test_config(), AddressSpace::clc_global);
}

TEMPLATE_LIST_TEST_CASE_CUSTOM_NAME("vstore_local", "", ca::VectorTypes,
                                    test_name<TestType>) {
  const TestConfig &config = get_test_config();
  using scalar_type = typename TestType::scalar_type;
  ...
  test_vstore<TestType>(get_test_config(), AddressSpace::clc_local);
}

TEMPLATE_LIST_TEST_CASE_CUSTOM_NAME("vstore_private", "", ca::VectorTypes,
                                    test_name<TestType>) {
  const TestConfig &config = get_test_config();
  using scalar_type = typename TestType::scalar_type;
  ...
  test_vstore<TestType>(get_test_config(), AddressSpace::clc_private);
}

TEMPLATE_LIST_TEST_CASE_CUSTOM_NAME("vstore_generic", "", ca::VectorTypes,
                                    test_name<TestType>) {
  const TestConfig &config = get_test_config();
  using scalar_type = typename TestType::scalar_type;
  ...
  test_vstore<TestType>(get_test_config(), AddressSpace::clc_generic);
}
