...
std::vector<OUTPUT_TYPE> get_reference(const std::vector<INPUT_TYPE> &input_a,
  ...
  for (size_t i = 0; i < output.size(); ++i) {
    if constexpr (ca::is_vector_v<INPUT_TYPE>) {
      output[i] = ca::equal_component_wise<OUTPUT_TYPE>(input_a[i], input_b[i]);
    } else {
...
TEMPLATE_TEST_CASE("isequal", "", ca::clc_float_t, ca::clc_float2_t,
  ...
  const ca::Kernel kernel =
      create_kernel("kernels/oclc_relational_functions/isequal.cl",
