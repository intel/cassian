...
kernel void
test_kernel_sub_group_shuffle_common_offset(__global uint *shuffle_results) {
  size_t tid = get_global_linear_id();
  const uint current_sub_group_size = get_sub_group_size();
  const uint offset = current_sub_group_size;
  ...
  for (uint i = 0; i < current_sub_group_size; i++) {
    results[i] = intel_sub_group_shuffle(val_to_shuffle, offset - 1);
  }

  uint passed = 1;
  for (uint i = 0; i < current_sub_group_size; i++) {
    if (USE_ALL(results[i] != (DATA_TYPE)(current_sub_group_size - 1))) {
      passed = 0;
      break;
    }
  }
