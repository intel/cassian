...
kernel void test_kernel_sub_group_shuffle_down(global uint *shuffle_results) {
  ...
  bool passed = false;
  new_value = intel_sub_group_shuffle_down(cur, next, DELTA_SIZE);
  if (SHUFFLE_INDEX < sub_group_size) {
    expected_value = (DATA_TYPE)(sub_group_local_id + DELTA_SIZE + 1);
  } else if (SHUFFLE_INDEX >= sub_group_size &&
             SHUFFLE_INDEX < 2 * sub_group_size) {
    expected_value = (DATA_TYPE)(SHUFFLE_INDEX - sub_group_size + 101);
  } else { // out of range
