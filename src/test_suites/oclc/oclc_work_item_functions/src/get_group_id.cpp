...
template <size_t N> void test_get_group_id(const TestConfig &config) {
  ...
  const ca::Kernel kernel =
      create_kernel("kernels/oclc_work_item_functions/get_group_id.cl",
...
TEST_CASE("get_group_id", "") {
  SECTION("1D") { test_get_group_id<1>(get_test_config()); }
  SECTION("2D") { test_get_group_id<2>(get_test_config()); }
  SECTION("3D") { test_get_group_id<3>(get_test_config()); }
}
