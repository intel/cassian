...
template <size_t N> void test_get_global_id(const TestConfig &config) {
...
TEST_CASE("get_global_id", "") {
  SECTION("1D") { test_get_global_id<1>(get_test_config()); }
  SECTION("2D") { test_get_global_id<2>(get_test_config()); }
  SECTION("3D") { test_get_global_id<3>(get_test_config()); }
}
