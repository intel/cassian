...
get_reference(const std::array<size_t, N> &global_work_size,
  ...
  for (size_t i = 0; i < N; ++i) {
    std::vector<uint32_t> enqueued_local_size(global_work_size.at(i),
...
TEST_CASE("get_enqueued_local_size", "") {
  SECTION("1D") { test_get_enqueued_local_size<1>(get_test_config()); }
  SECTION("2D") { test_get_enqueued_local_size<2>(get_test_config()); }
  SECTION("3D") { test_get_enqueued_local_size<3>(get_test_config()); }
}
