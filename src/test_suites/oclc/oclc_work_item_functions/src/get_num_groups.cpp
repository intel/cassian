...
get_reference(const std::array<size_t, N> &global_work_size,
  ...
  std::array<std::vector<uint32_t>, N> output = {};
  for (size_t i = 0; i < N; ++i) {
    std::vector<uint32_t> num_groups(global_work_size.at(i),
                                     work_group_count.at(i));
    output.at(i) = num_groups;
  }
  return output;
}
...
template <size_t N> void test_get_num_groups(const TestConfig &config) {
  ...
  const ca::Kernel kernel =
      create_kernel("kernels/oclc_work_item_functions/get_num_groups.cl",
...
TEST_CASE("get_num_groups", "") {
  SECTION("1D") { test_get_num_groups<1>(get_test_config()); }
  SECTION("2D") { test_get_num_groups<2>(get_test_config()); }
  SECTION("3D") { test_get_num_groups<3>(get_test_config()); }
}
