...
test_kernel_1(global uint *output) {
  ...
  output[tid] = (uint)get_enqueued_local_size(0);
};
...
test_kernel_2(global uint *output_x, global uint *output_y) {
  ...
  output_x[tid_x] = (uint)get_enqueued_local_size(0);
  output_y[tid_y] = (uint)get_enqueued_local_size(1);
};
...
test_kernel_3(global uint *output_x, global uint *output_y,
  ...
  output_x[tid_x] = (uint)get_enqueued_local_size(0);
  output_y[tid_y] = (uint)get_enqueued_local_size(1);
  output_z[tid_z] = (uint)get_enqueued_local_size(2);
};
