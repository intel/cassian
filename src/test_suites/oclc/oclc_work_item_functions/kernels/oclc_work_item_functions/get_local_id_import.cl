...
size_t get_local_id_wrapper(uint n);

#ifdef SIMD
__attribute__((intel_reqd_sub_group_size(SIMD)))
#endif // SIMD
kernel void
test_kernel_wrappers_1(global uint *output) {
  const size_t tid = get_global_id(0);
  output[tid] = (uint)get_local_id_wrapper(0);
};
...
#endif // SIMD
kernel void
test_kernel_wrappers_2(global uint *output_x, global uint *output_y) {
