...
constexpr double sqrt_margin = std::numeric_limits<float>::epsilon();
...
TEMPLATE_TEST_CASE("cm_sqrt(vector)", "[cm][sqrt]", float, half) {
  ...
  SECTION("--sat-no-limits") {
    test_math<TestType>(
        ...
        Saturation::USE_SAT_NOLIMITS, InputType::vector, sqrt_margin);
  }
}
...
TEMPLATE_TEST_CASE("cm_sqrt(matrix)", "[cm][sqrt]", float, half) {
  ...
  SECTION("--sat-no-limits") {
    test_math<TestType>(
        ...
        0, static_cast<float>(std::numeric_limits<TestType>::max()),
        Saturation::USE_SAT_NOLIMITS, InputType::matrix, sqrt_margin);
  }
}
...
TEMPLATE_TEST_CASE("cm_sqrt(scalar)", "[cm][sqrt]", float, half) {
  ...
  SECTION("--sat-no-limits") {
    test_math<TestType>(
        ...
        0, static_cast<float>(std::numeric_limits<TestType>::max()),
        Saturation::USE_SAT_NOLIMITS, InputType::scalar, sqrt_margin);
  }
}
...
TEMPLATE_TEST_CASE("cm_sqrt(const)", "[cm][sqrt]", float, half) {
  ...
  SECTION("--sat-no-limits") {
    test_math<TestType>(
        ...
        0, static_cast<float>(std::numeric_limits<TestType>::max()),
        Saturation::USE_SAT_NOLIMITS, InputType::constant, sqrt_margin);
  }
}