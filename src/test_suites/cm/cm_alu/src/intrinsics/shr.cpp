...
TEMPLATE_LIST_TEST_CASE("cm_shr", "[cm][intrinsics][shift]", bitwise_list_t) {
  ...
  SECTION("cm_shr(Vector, Vector)") {
    VectorGenerator<src0_t, src1_t> gen(reference, is_overflow, gen0, gen1);
    test_case_t(config.runtime(), config.program_type(), "cm_shr", gen,
...

  {
    ...
    SECTION("cm_shr(Vector, Constant)") {
      test_case_t(config.runtime(), config.program_type(), "cm_shr", gen,
                  OperandType::vector,
